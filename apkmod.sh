...
update() {
	temp=$(curl -L -s https://github.com/evildevill/Apkmod/raw/master/apkmod.sh | grep -w "VERSION=" | head -n1)
	...
	if [ "${1}" = "-u" ]; then
		...
		wget https://raw.githubusercontent.com/evildevill/Apkmod/master/setup.sh && sh setup.sh $ARGS
	fi
}
