...
install_deps_kali() {
	...
	wget https://github.com/evildevill/Apkmod/raw/master/apkmod.sh -O $PREFIX/bin/apkmod && chmod +x $PREFIX/bin/apkmod
	printf "[*] Done\n"
}

setup_alpine() {
	if [ ! "$1" = "--without-alpine" ]; then
		wget https://raw.githubusercontent.com/evildevill/Apkmod/master/scripts/TermuxAlpine.sh -O TermuxAlpine.sh
...
install_deps() {
	...
	wget https://github.com/evildevill/Apkmod/raw/master/apkmod.sh -O ${BINDIR}/apkmod
	chmod +x ${BINDIR}/apkmod
	chmod +x ${ALPINEDIR}/usr/bin/aapt
	chmod +x ${ALPINEDIR}/usr/bin/aapt2
	rm -rf ~/.apkmod && mkdir -p ~/.apkmod/framework
    wget https://github.com/evildevill/Apkmod/raw/master/apkmod.p12 -O ~/.apkmod/apkmod.p12
}

install_scripts() {
	for script in apktool_termux.sh apktool_alpine.sh apk.rb jadx_termux.sh jadx_alpine.sh; do
		wget https://github.com/evildevill/Apkmod/raw/master/scripts/${script} -O ${script}
	done
